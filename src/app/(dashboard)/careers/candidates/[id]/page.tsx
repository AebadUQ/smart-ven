...
import { getAllCandidatesById, getGeneralCandidates, getJobStatus, getActiveJobs } from '@/services/jobs.api';
...
export default function Page({ searchParams }: PageProps): React.JSX.Element {
  ...
  const id = params?.id;
  const url = id === "general" ?  getGeneralCandidates() : getAllCandidatesById(id.toString())
  ...
  const fetchStatus = useCallback(async () => {
    ...
    const fetchJobs = useCallback(async () => {
      try {
        const getJobs = await getActiveJobs();
  ...
  return (
    <Box sx={{ bgcolor: 'var(--mui-palette-background-level1)', p: 3 }}>
      <Stack spacing={3}>
        ...
        <Box >
            <Typography variant="h5">{id === "general" ? "General Applicants" : "Applicants"}</Typography>
          </Box>
        ...
        <CustomersSelectionProvider customers={[]}>
          <Card>
            <CandidatesFilters filterType="specific" jobStatus={jobStatus} jobType={jobs} filters={filter} setFilters={setFilter}  />
            <Divider />
     ...
     { (<AssignVacancyModal key={`assign2-${selectedRow?.id}`} open={assignVacancyOpen} onClose={() => setAssignVacancyOpen(false)}  status={jobs} setData={setData} data={selectedRow} candidateType={id === "general" ? "general": "candidate"} /> )}
